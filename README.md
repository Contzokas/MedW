...
<h3 align="center">AI-Powered Medical Triage for the Greek National Health System</h3>

<p align="center">
  <a href="https://github.com/Contzokas/MedW/actions/workflows/deploy.yml"><img src="https://img.shields.io/github/actions/workflow/status/Contzokas/MedW/deploy.yml?branch=main&label=CI%2FCD&style=flat-square" alt="CI/CD"></a>
  ...
  <a href="#"><img src="https://img.shields.io/badge/GDPR-compliant-green?style=flat-square" alt="GDPR"></a>
  <a href="#"><img src="https://img.shields.io/badge/NVIDIA-B200-76b900?style=flat-square&logo=nvidia" alt="NVIDIA B200"></a>
</p>

---

> **Kiefer AI Open Hackathon 2026** — Built in 24 hours. Deployed on NVIDIA B200 GPUs via Run:ai. Solving Greece's ~7M annual misdirected appointments.

---

## The Problem

Every year, **~7 million outpatient appointments** in the Greek NHS (ΕΣΥ) are directed to the wrong medical specialty — because patients self-refer without clinical guidance. This clogs emergency departments, inflates wait times, and delays critical care.
...
| Feature | Description |
|---------|-------------|
| **Symptom Wizard** | Guided step-by-step mode for patients who need help articulating symptoms — body area → symptoms → severity → duration |
...
| **Patient Profiler** | Optional medical history (age, conditions, medications, allergies) stored only in the browser — injected into the LLM prompt for better accuracy |
| **Geolocation** | Find the nearest available doctor by real distance (Haversine formula) |
| **Uncertain Result Fallback** | When the AI cannot reliably classify, it honestly admits uncertainty rather than guessing — no false confidence |
...
| **Embeddings** | nv-embedqa-e5-v5 (NIM) / all-MiniLM-L6-v2 (local) | 512-token context, English-optimized; encodes Greek symptoms after translation |
| **Vector DB** | Milvus Lite (prod) / ChromaDB 1.5.7 (local) | Embedded in the backend process — no separate service needed |
| **Reranker** | Cross-encoder (NVIDIA nv-rerankqa) | Reranks top-9 retrieved chunks → top-3 most relevant before LLM prompt assembly |
...
| **Infrastructure** | Docker Compose · GitHub Actions · Run:ai REST API · GHCR | CI/CD from push to deploy in <15 min; GPU scheduling via Run:ai |
...
|------|---------|----------|
| 1 | Normal operation | Full AI triage with RAG + LLM + doctor match |
...
- **Immutable tags:** Every deploy gets a sha-pinned tag (`latest-abc1234`) for rollback
