...
function HomeContent() {
  ...
  const editProfile = searchParams.get("profile") === "1"
  ...
  const onboardingWasOpenRef = useRef(false)
  ...
  const [onboardingComplete, setOnboardingComplete] = useState(() => {
    try { return localStorage.getItem("medw_onboarding_done") === "1" }
    catch { return false }
  })
  ...
  // Detect when onboarding tour finishes during this session
  useEffect(() => {
    if (onboarding.isOpen) {
      onboardingWasOpenRef.current = true
    } else if (onboardingWasOpenRef.current) {
      setOnboardingComplete(true)
    }
  }, [onboarding.isOpen])

  // Show modal on first visit (no cookie found) — wait for onboarding to finish
  useEffect(() => {
    if (isProfileLoaded && profile === null && onboardingComplete) {
      setShowProfiler(true)
    }
  }, [isProfileLoaded, profile, onboardingComplete])

  // Manual toggle via ?profile=1 — wait for onboarding to finish
  useEffect(() => {
    if (editProfile && !onboarding.isOpen) {
      setShowProfiler(true)
    }
  }, [editProfile, onboarding.isOpen])
